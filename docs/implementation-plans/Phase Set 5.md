# Implementation Plan — Phase Set 5: Security Hardening, Bilingual STT & Academic Paper

This plan details the design and execution steps for the Security Hardening, Bilingual Speech-to-Text, and Academic Benchmarking phases. We implement strict runtime security guards, auto-detected bilingual STT routing with cloud fallback for Hindi, automated concurrent/unload race tests, long-running soak-testing profiles, full benchmarks, and the final academic research paper.
...
> [!IMPORTANT]
> - **Bilingual STT Cloud Fallback**: Direct auto-routing to the Sarvam AI API for Hindi speech streams means audio data leaves the local device when Hindi is spoken. This is a documented, user-approved exception to the local-first architecture and is highlighted in the research paper's security and privacy sections.
> - **Model Agnosticism in Prompting**: XML tool calls from the brain in response to Hindi inputs are generated in pure ASCII JSON (avoiding Devanagari inside the JSON payload) to ensure that the regex-based `MCPToolServer` parsing remains fully stable and compatible.
...
* If Hindi (`hi`) is detected and the Sarvam API key is configured, post the audio bytes to the Sarvam API endpoint (`https://api.sarvam.ai/speech-to-text`) in `mode="transcribe"` for high-precision Hindi text synthesis.
...
* Execute the complete testing suite to guarantee that all unit and integration tests pass flawlessly:
  ```bash
  pytest tests/ -v
  ```
